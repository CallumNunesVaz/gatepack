...
def test_sby_file_structure():
    tasks = props.property_tasks(_compiled())
    text = props.build_sby_file("t", "build/generated.v", "build/properties.sv", tasks)
    assert "read_verilog -sv build/generated.v" in text
    assert "read_verilog -sv build/properties.sv" in text
    assert "prep -top t_properties" in text
    assert "smtbmc z3" in text
    assert "[gp_assert_0]" in text
...
def test_run_properties_with_sby_present_passes(tmp_path):
    ...
    assert runner.argv == ["sby", "-f", str(tmp_path / "properties.sby")]
