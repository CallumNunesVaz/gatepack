...
def emit_properties(compiled: CompiledDesign) -> str:
    ...
    clock_name = design.clock.signal if design.clock else "clk"
    reset_name = reset.signal
    # `reset_held` is the expression that is true while reset is ASSERTED;
    # `reset_released` is its complement, and replaces SVA's `disable iff`.
    ...
    lines.append("//")
    lines.append("// This is a FORMAL harness, not a simulation testbench: no")
    lines.append("// `initial`, no delays, no clock generator. The clock and")
    lines.append("// reset are free inputs the engine drives, and the only")
    lines.append("// constraint on them is the reset assumption below.")
    lines.append("")
    ...
    # never reach after reset and a *true* invariant fails at step 1.
    ...
    lines.append("  reg gp_past_valid = 1'b0;")
    lines.append(f"  always @(posedge {clock_name}) gp_past_valid <= 1'b1;")
    lines.append(
        f"  always @(posedge {clock_name}) if (!gp_past_valid) assume ({reset_held});"
    )
    lines.append("")
    ...
    for index, prop in enumerate(design.properties):
        ...
        if prop.kind in ("invariant", "mutex"):
            ...
            lines.append("    if (gp_past_valid) begin")
            ...
            lines.append(f"  always @(posedge {clock_name}) begin")
            lines.append(f"    if (gp_past_valid && {reset_released})")
