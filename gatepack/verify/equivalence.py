...
def build_equivalence_script(
    ...
    lines = [
        ...
        f"write_verilog -noattr {yosys.script_path(config.gold_v)}",
        "design -reset",
        "# --- golden side, round-tripped: re-proc is mandatory after write_verilog ---",
        f"read_verilog {yosys.script_path(config.gold_v)} {yosys.script_path(config.cells_spec_v)}",
        ...
        f"read_verilog {yosys.script_path(config.gate_v)} {yosys.script_path(config.cells_sim_v)}",
        "proc; flatten; opt; async2sync; opt",
    ...
    *bounded pass*, never a green pass (§21.5).  The golden side is the *golden
    ...
    return "\n".join(
        [
            ...
            "",
            "[script]",
            f"read_verilog {yosys.script_path(config.gold_v)}",
            "proc; opt; async2sync; opt",
            f"rename {config.top} golden",
            f"read_verilog {yosys.script_path(config.mapped_v)} {yosys.script_path(config.cells_sim_v)}",
            "proc; flatten; opt; async2sync; opt",
