 ...
 * docs/MILESTONE-AUDIT.md [GUI-5] happened: the schematic told a GUI user to "run
...
export interface PipelineInputs {
  ...
  /**
   * Whether the filesystem says a source is newer than the build, or null when
   * it cannot say. This is *measured*, and it outranks `buildRevision`, which
   * can only ever be observed: artefacts carry no revision, so a project opened
   * with an already-stale build would look fresh until the user next typed.
   */
...
export function computeStageStates(inputs: PipelineInputs): StageStatus[] {
  ...
  const build: StageStatus = (() => {
    ...
    // The filesystem's answer first, because it is a measurement. A build that
    // predates the spec is stale however recently this session observed it.
    if (inputs.sourcesNewerThanBuild === true) return { stageId: 'build', state: 'stale' };
    // Then edits made since we looked. `buildRevision` cannot see a build that
    // was already stale when the project opened, which is why it is second.
